#!/usr/bin/env node
/**
 * CHAMP-02 screenshot driver (issue #86): serves the repo through vite, races
 * champ-1 headlessly in a real browser page and captures the acceptance views
 * into tests/artifacts/. Usage: node --import tsx scripts/champ1-shots.mjs
 */
...
await page.goto(`${baseUrl}/scripts/champ1-shots/index.html`);
...
for (const view of ['overview', 'fork', 'ferry', 'shortcut', 'battle', 'phone']) {
  const file = join(out, `champ1-${view}.png`);
  await page.locator(`canvas[data-view="${view}"]`).screenshot({ path: file });
  console.log(`wrote tests/artifacts/champ1-${view}.png`);
}
