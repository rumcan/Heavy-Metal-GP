#!/usr/bin/env node
/**
 * RK-05 — derive the ranked badges from ONE painted master.
 *
 ...
 *
 * Master: `assets/ui/rank/medallion-master.png` — a painted iron-and-brass
 * campaign medallion (gear ring, laurel, riveted octagon) with a BLANK centre
 * disc, on a dark painted ground. It is the hand-swappable source: replace it
 * with a re-painted plate of the same shape and every badge below is
 * regenerated to match.
 *
 * Copied from HexMatch's `tools/make-rank-badges.mjs` (RANK-01 #147) — the
 * pipeline, the crop and the pip layout are that file's; only the TIER TABLE is
 * this game's (see `RANK_TIERS` in `src/net/rating.ts`). The reasoning for one
 * master over seven paintings is theirs and still true:
 *
 *   - six separate paintings of the same medal drift (different rims,
 *     different lighting, different crop), and the ladder is exactly the place
 *     where the tiers must read as ONE family at a glance;
 *   - the medal is already the right shape for a game about a gear-driven
 *     marble racer in a mine: the gear ring and the rivets are the kit.
 *
 * So the family is derived from a single plate:
 *
 *   knock  — the painted background becomes alpha (`blackKnock`), so a badge
 *            is a cut-out and drops onto any panel of the theme;
 *   tint   — each tier is the SAME luminance re-metalised (grey, bronze, iron,
 *            steel, gold, crimson). Metal IS luminance in this theme, so one
 *            tint per tier is the whole difference;
 ...
 *
 * `unranked` is the master's own iron with no pips: the badge for a driver who
 * has not finished a rated race yet. It is a state, not a tier — see
 * `rankKeyOf` in `src/net/rating.ts`.
 *
...
const SRC = join(ROOT, "assets/ui/rank");
...
/** Painted at 96px so the CSS never scales a raster UP (the chip prints it at 24–30). */
const SIZE = 96;
 ...
 *
 * The LADDER ORDER IS THE PIP COUNT, and it is the one thing that must not
 * drift: a driver climbing the ladder sees one more star each time, whatever
 * the metal name says. Kept in step with `RANK_TIERS` in `src/net/rating.ts`
 * by `tests/rating.test.ts` (which reads this table and the files on disk).
 */
const TIERS = [
  { key: "unranked", label: "Unranked", tint: [92, 86, 78], pips: 0, pip: "#8d8578", dim: true },
  { key: "scrap", label: "Scrap", tint: [132, 124, 112], pips: 1, pip: "#e6dcc8" },
  { key: "bronze-bolt", label: "Bronze Bolt", tint: [176, 116, 62], pips: 2, pip: "#ffdcae" },
  { key: "iron", label: "Iron", tint: [118, 128, 140], pips: 3, pip: "#e4edf5" },
  { key: "steel", label: "Steel", tint: [178, 190, 200], pips: 4, pip: "#f4f8fb" },
  { key: "gold-gear", label: "Gold Gear", tint: [212, 168, 78], pips: 5, pip: "#fff3c9" },
  { key: "heavy-metal", label: "Heavy Metal", tint: [188, 78, 62], pips: 6, pip: "#ffd98a" },
];
...
async function contact(path) {
  ...
  const mk = [
    {
      input: {
        create: { width: w, height: cell * 2, channels: 4, background: { r: 8, g: 20, b: 32, alpha: 1 } },
      },
      left: 0,
      top: 0,
    },
