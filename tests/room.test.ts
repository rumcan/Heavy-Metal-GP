...
import { Clock } from '@series-inc/rundot-game-sdk/mp-server';
...
class TestRoom extends RaceRoom {
...
function setup(config: Record<string, unknown> = {}, roomId = 'room-1'): Harness {
  const frames: Frame[] = [];
...
function messageOf(frame: Frame): RaceProtocol {
  return { ...(frame.data as Record<string, unknown>), type: frame.type } as RaceProtocol;
}

const to = (frames: Frame[], target: string): Frame[] => frames.filter((f) => f.target === target);
const ofType = (frames: Frame[], type: string): Frame[] => frames.filter((f) => f.type === type);
const broadcasts = (frames: Frame[]): RaceProtocol[] => frames.filter((f) => f.target === 'broadcast').map(messageOf);
const sentTo = (frames: Frame[], playerId: string): RaceProtocol[] => to(frames, playerId).map(messageOf);
 ...
 */
function welcomeOf(frames: Frame[], target = 'broadcast'): WelcomeMsg {
...
async function join(h: Harness, id: string, username = id.toUpperCase()): Promise<Player> {
...
async function joinRefused(h: Harness, id: string): Promise<string> {
...
async function send(h: Harness, from: string, msg: RaceProtocol): Promise<void> {
  const { type, ...rest } = msg as { type: string } & Record<string, unknown>;
  await h.protocol.handleMessage(from, type, rest);
}

async function leave(h: Harness, id: string, reason: LeaveReason = 'leave'): Promise<void> {
